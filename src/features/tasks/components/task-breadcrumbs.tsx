...
export const TaskBreadcrumbs = ({ project, task }: TaskBreadcrumbsProps) => {
  ...
  return (
    <div className="flex items-center gap-x-2  bg-white rounded-2xl  backdrop-blur-lg border border-white/30 shadow-2xl p-2">
      <ConfirmDialog />
      ...
      <Button disabled={isPending} onClick={handleDeleteTask} className="ml-auto bg-red-600" variant="destructive" size="sm">
        <Trash className="size-4 lg:mr-2" />
        <span className="hidden lg:block bg-red-600">Delete task</span>
      </Button>
    </div>
  );
};
